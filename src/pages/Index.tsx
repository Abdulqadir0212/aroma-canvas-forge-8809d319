...
const SectionFallback = () => (
  <div className="py-12 text-center">
    <p className="font-body text-muted-foreground">This section could not be loaded.</p>
  </div>
);
    ...
    <div className="container mx-auto px-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {highlights.map((h, i) => (
          <div
            key={h.label}
            className="animate-fade-in-up flex items-center gap-3 justify-center"
...
const BestsellersSection = () => {
  const { data: bestsellers, isLoading, isError } = useBestsellers();

  return (
    <section className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <p className="text-gold font-body tracking-widest uppercase text-xs mb-3 font-medium">Featured</p>
          <h2 className="font-heading text-3xl md:text-4xl font-bold text-secondary mb-4">Our Bestsellers</h2>
          ...
          {isLoading
            ...
            ? (
              <div className="col-span-full text-center py-12">
                <p className="font-body text-muted-foreground">Could not load products. Please try again later.</p>
              </div>
            )
...
const BrandStorySection = () => {
  const { data: brandStory, isLoading } = useSiteContent("brand_story");

  return (
    <section className="py-20 bg-card">
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-gold font-body tracking-widest uppercase text-xs mb-3 font-medium">Our Legacy</p>
          <h2 className="font-heading text-3xl md:text-4xl font-bold text-secondary mb-6">
            {isLoading ? <Skeleton className="h-10 w-64 mx-auto" /> : brandStory?.title ?? "A Trusted Fragrance House"}
          </h2>
          {isLoading ? (
          ...
          <Link
            to="/about"
            className="inline-flex items-center gap-2 text-gold font-body font-semibold hover:underline"
          >
...
const WhyChooseUsSection = () => {
  const { data: reasons, isLoading } = useWhyChooseUs();

  return (
    <section className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <p className="text-gold font-body tracking-widest uppercase text-xs mb-3 font-medium">Why Us</p>
          <h2 className="font-heading text-3xl md:text-4xl font-bold text-secondary">The M M Attarwala Advantage</h2>
        </div>
        ...
        <div className="max-w-3xl mx-auto grid sm:grid-cols-2 gap-4">
          {isLoading
            ...
            : reasons?.map((r, i) => (
                <div
                  key={r}
                  className="animate-fade-in-up flex items-start gap-3 bg-card rounded-xl p-5 border border-border"
...
const ReviewsSection = () => {
  const { data: reviews, isLoading } = useTestimonials(3);

  return (
    <section className="py-20 bg-card">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <p className="text-gold font-body tracking-widest uppercase text-xs mb-3 font-medium">Reviews</p>
          <h2 className="font-heading text-3xl md:text-4xl font-bold text-secondary">What Our Customers Say</h2>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto mb-10">
          {isLoading
            ...
            : reviews?.map((r, i) => (
                <div
                  key={r.id}
                  className="animate-fade-in-up bg-background rounded-xl p-6 border border-border"
                  style={{ animationDelay: `${i * 80}ms`, animationFillMode: 'forwards' }}
                >
...
const CTASection = () => (
  <section className="py-20 bg-secondary">
    <div className="container mx-auto px-4 text-center">
      ...
      <p className="font-body text-primary-foreground/60 max-w-md mx-auto mb-8">
        Explore our curated collection of attars, perfumes, and aromatic compounds.
      </p>
      <Link
        to="/products"
        className="inline-flex items-center gap-2 bg-gold text-primary-foreground font-body font-semibold px-8 py-3.5 rounded-lg hover:opacity-90 transition-opacity text-base"
      >
        <ShoppingBag size={18} />
        Shop Now
      </Link>
